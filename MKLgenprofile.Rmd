---
title : "The Mae Klong Hydro Power Plant (MKL) generation profile"
...
```{r mklgenprofile2021, echo=FALSE, message=FALSE, warning=FALSE}
mklprofile2021 <-
  read_excel("rawdata/MKL_01012021_31122021.xlsx") %>% 
  rename_all(tolower) %>% 
...
```
```{r mklgenprofile2022, echo=FALSE, message=FALSE, warning=FALSE}
mklprofile2022 <-
  read_excel("rawdata/MKL_01012022_31122022.xlsx") %>% 
  rename_all(tolower) %>% 
  mutate(timestamp = as.POSIXct(timestamp, format = "%d-%b-%Y %H:%M", tz = "UTC"),
         date = as.Date(timestamp, format = "%d-%b-%Y"),
         time = format(as.POSIXct(timestamp), format = "%H:%M:%S"), 
...
```{r plot_mklminegtnetgen2021, echo=FALSE, warning=FALSE, message=FALSE}
ggplotly(
  mklprofile2021 %>% 
